...
public class PreRetrievalController {

    private final ChatClient chatClient;
    private final VectorStore vectorStore;

    private final ChatClient webSerachChatClient;
    ...
    @Autowired
    public PreRetrievalController(@Qualifier("chatMemoryChatClient") ChatClient chatClient,
                                  VectorStore vectorStore,
                                  @Qualifier("webSearchRAGChatClient") ChatClient webSerachChatClient) {
        this.chatClient = chatClient;
        this.vectorStore = vectorStore;
        this.webSerachChatClient = webSerachChatClient;
    }
    ...
    public ResponseEntity<String> documentChat(@RequestHeader(value = "username") String userName,
        ...
        String answer = chatClient.prompt()
                ...
                .user(message)
                .call().content(); // Spring AI 的 RetrievalAugmentationAdvisor 會攔截這個請求，開始跑整個 RAG pipeline。

        return ResponseEntity.ok(answer);
    }

    @GetMapping("web-search/chat")
    public ResponseEntity<String> webSearchChat(@RequestHeader(value = "username") String userName,
                                                @RequestParam(value = "message") String message) {
        String answer = webSerachChatClient.prompt()
                .advisors(a -> a.param(ChatMemory.CONVERSATION_ID, userName))
                .user(message)
                .call().content();
